...
import { X, Check, Mail, Lock, User, AlertCircle } from "lucide-react";
  ...
  const handleAuth = async () => {
    ...
    try {
      if (authMode === "login") {
        ...
        const res = await auth.login(email, password);
        setAuth(res.user, res.access_token, res.refresh_token);
        toast.success("Account created! Welcome to BookToPaper.");
      }
        ...
        <div className="flex-1 p-8 md:p-12 bg-bg-surface flex flex-col overflow-hidden">
            ...
            <div className="flex bg-bg-raised p-1 rounded-xl w-fit">
              <button
                ...
                className={cn(
                  ...
                  authMode === "login"
                    ? "bg-white shadow-sm text-accent-primary"
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="space-y-4" onKeyDown={handleKeyDown}>
                {authMode === "signup" && (
                  <div className="space-y-1.5">
                    <label className="text-13px font-medium text-text-secondary pl-1">Full Name</label>
                ...
                <div className="space-y-1.5">
                  <label className="text-13px font-medium text-text-secondary pl-1">Email Address</label>
                  <div className="relative">
                ...
                <div className="space-y-1.5">
                  <label className="text-13px font-medium text-text-secondary pl-1">Password</label>
                  <div className="relative">
                    ...
                    />
                    {authMode === "login" && (
                      <button className="absolute right-3 top-1/2 -translate-y-1/2 text-[11px] font-bold text-accent-primary hover:underline uppercase tracking-wider">
                        Forgot?
                      </button>
                    )}
                  </div>
              ...
              {/* Error message */}
              {error && (
                <div className="mt-4 flex items-center gap-2 text-red-500 text-sm bg-red-50 border border-red-100 rounded-xl p-3">
              ...
              <Button
                onClick={handleAuth}
                disabled={isLoading}
                className="mt-6 h-12 text-lg rounded-xl"
              >
                {isLoading
                  ? "Processing..."
                  : authMode === "login"
              ...
              <div className="flex gap-4">
                <Button
                  ...
                  onClick={handleAuth}
                  disabled={isLoading}
                  className="flex-1 h-12 flex items-center justify-center gap-2 border-border/50"
                >
              ...
              <p className="mt-auto pt-4 text-center text-xs text-text-secondary">
                {authMode === "login"
